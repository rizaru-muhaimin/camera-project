...
class CameraWindow(QMainWindow):
        ...
        if selected is not None:
            for position in range(self.camera_select.count()):
                if self.camera_select.itemData(position) == selected:
                    ...
                    break
        self.set_status(f"Found {self.camera_select.count()} camera(s)")

    def set_status(self, message: str, level: str = "normal") -> None:
        self.status.setText(message)
        self.status.setProperty("statusLevel", level)
    ...
    def open_camera(self) -> None:
        ...
        if not isinstance(camera, CameraDescriptor):
            self.set_status("No camera selected", "error")
            return
        if self.camera.open(camera):
            ...
            self.set_status(f"{camera.display_name} connected", "success")
        else:
            detail = self.camera.last_error or "camera access failed"
            self.set_status(
                f"Could not open {camera.display_name}: {detail}", "error"
            )
    ...
    def close_camera(self) -> None:
        ...
        self.set_status("Camera stopped", "success")

    def update_frame(self) -> None:
        frame = self.camera.read()
        if frame is None:
            self.set_status(self.camera.last_error or "No frame received", "error")
            return
        ...
        if frame.ndim == 2:
            ...
            self.set_status(f"Unsupported camera frame shape: {frame.shape}", "error")
            return
        image = image.copy()
    ...
    def capture_photo(self) -> None:
        if self.current_frame is None:
            self.set_status("Open a camera before capturing", "error")
            return
        self.save_settings()
        ...
        try:
            ...
            self.set_status(f"Could not save photo: {error}", "error")
